...
if(strlen($_SESSION['alogin'])==0)
	...
	{
		$did=intval($_GET['del']);
		$sql = "delete from medicine_unit WHERE  ID=:did AND shopId=:shopId AND branchId=:branchId";
	...
	if(isset($_POST['submit']))
		...
		if(isset($_POST['update']))
			...
			$status=$_POST['radio_value'];
			$id=$_POST['id'];
			$sql="UPDATE medicine_unit SET MedicineUnit=:category, Status=:radio_value WHERE ID=:id AND shopId=:shopId AND branchId=:branchId";
			$query = $dbh->prepare($sql);
                            ...
                            <?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<tbody>
                                        ...
                                        <?php $sql = "SELECT * from  medicine_unit WHERE shopId=:shopId";
                                        $query = $dbh -> prepare($sql);
										$query->bindParam(':shopId',$_SESSION['user']['shopId'],PDO::PARAM_STR);
                                        $query->execute();
										...
										<tr>
											...
											<td><?php echo htmlentities($result->MedicineUnit);?></td>
											...
											<p type="button"  class="px-2 btn btn-info"  onClick="edit_unit(this.id)" id="unit-<?php echo htmlentities($result->ID);?>"><i class="fas fa-edit" aria-hidden="true"></i></p> 
                                            <a  class="px-2" href="#" > <i class="fas fa-eye" aria-hidden="true"></i></a> 
											<a  class="px-2" href="products_unit_list.php?del=<?php echo htmlentities($result->ID);?>" onclick="return confirm('Do you really want to delete this record')"> <i style="color: red;" class="far fa-trash-alt" aria-hidden="true"></i></a>
											</td>
										</tr>
										<?php $cnt=$cnt+1; }} ?>
									</tbody>
