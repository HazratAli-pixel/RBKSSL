...
<?php
	...
	else
	{
		...
		{
			$did=intval($_GET['del']);
			$sql = "delete from reference WHERE ID=:did AND shopId=:shopId AND branchId=:branchId";
		...
		if(isset($_GET['close'])){    
			$cmpid=$_GET['close'];
			$sts=0;
			$sql ="update reference set Status=:status where ID=:cusId AND shopId=:shopId AND branchId=:branchId";
			$query = $dbh->prepare($sql);
							...
							<div  class="card-header">
                                <div class="d-flex justify-content-between align-items-center h-100px">
									...
									<div >
                                        ...
                                        <a href="add_customer.php" class="btn btn-info mr-3"><i class="fas fa-plus mr-2" style="margin-right: 10px;"></i> Add Customer</a>                                              
